...
df <- data.frame(
  ...
  DELTA_COH = cut(rescale_variable(df.def$DELTA_COH, 0, 100), breaks = seq(0, 100, by = 5), include.lowest = TRUE),
  RESTORE = df.def$RESTORE
)

# Convertir las variables a factores
df$ESCENARIO <- as.factor(df$ESCENARIO)
df$DELTA_COH <- as.factor(df$DELTA_COH)
...
hchart(
  sankey_df,
  type = "sankey",
  name = "Pesos de Delta_Coh - Restoration planner"
)  |>
  hc_title(text = "<b>Diagrama de Sankey</b>") |>
  hc_subtitle(text = "<i>DELTA_COH - dataset Pinus halepensis</i>") |>
